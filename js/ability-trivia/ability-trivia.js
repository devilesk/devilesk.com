$(function () {
	...
	function generateQuestion() {
		...
		if ('cooldown' in rand_ability && rand_ability.cooldown.length > 0 && _.find(rand_ability.cooldown, function(num){ return num != 0; }) != undefined) {
			attr.push({tooltip:'COOLDOWN:',value:rand_ability.cooldown});
		}
		if ('manacost' in rand_ability && rand_ability.manacost.length > 0 && _.find(rand_ability.manacost, function(num){ return num != 0; }) != undefined) {
	...
	$.getJSON("/media/js/herodata.json", function (data) {
		...
		for (h in herodata) {
			var abilities = _.map(herodata[h].abilities, function (a) {
				return a.name;
			});
			for (var i = 0; i < abilities.length; i++) {
				var a = _.findWhere(herodata[h].abilities, {
					name: abilities[i]
				});
